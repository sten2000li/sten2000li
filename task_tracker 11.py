...
def add_task(task, category="Без категории", priority="Средний"):
    """Добавляет задачу с категорией и приоритетом / Adds a task with a category and priority"""
    if not task.strip():
    ...
    if priority not in priorities:
        priority = "Средний"
    tasks.append({"task": task, "completed": False, "category": category, "priority": priority})
...
def view_tasks():
    ...
    for i, task in enumerate(tasks, 1):
        ...
        print(f"{i}. [{status}] {task['task']} (категория: {task['category']}, приоритет: {task['priority']})")
...
def view_incomplete_tasks():
    ...
    for i, task in enumerate(tasks, 1):
        if not task["completed"]:
            print(f"{i}. [ ] {task['task']} (категория: {task['category']}, приоритет: {task['priority']})")
            found = True
    if not found:
