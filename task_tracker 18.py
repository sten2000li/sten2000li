...
def add_task(task, category="Без категории", priority="Средний", notes=""):
    """Добавляет задачу с категорией, приоритетом, датой создания и заметками / Adds a task with a category, priority, creation date, and notes"""
    if not task.strip():
...
def edit_task(task_index):
    ...
    if 1 <= task_index <= len(tasks):
        ...
        print(f"Текущая задача: {task['task']} (категория: {task['category']}, приоритет: {task['priority']}, создано: {task['created_at']}, заметки: {task['notes'] or 'Нет'})")
        ...
        print(f"Задача обновлена: '{task['task']}' (категория: {task['category']}, приоритет: {task['priority']}, заметки: {task['notes'] or 'Нет'}). / Task updated: '{task['task']}' (category: {task['category']}, priority: {task['priority']}, notes: {task['notes'] or 'None'}).")
    else:
...
def view_tasks(sort_by="priority"):
    ...
    for i, task in enumerate(sorted_tasks, 1):
        ...
        print(f"{i}. [{status}] {task['task']} (категория: {task['category']}, приоритет: {task['priority']}, создано: {task['created_at']}, заметки: {task['notes'] or 'Нет'})")
...
def view_incomplete_tasks(sort_by="priority"):
    ...
    if not sorted_tasks:
        ...
        return
    for i, task in enumerate(sorted_tasks, 1):
        print(f"{i}. [ ] {task['task']} (категория: {task['category']}, приоритет: {task['priority']}, создано: {task['created_at']}, заметки: {task['notes'] or 'Нет'})")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            add_task(task, category if category else "Без категории", priority, notes)
        elif choice == "2":
            view_tasks(sort_by)
        elif choice == "3":
            view_tasks(sort_by)
